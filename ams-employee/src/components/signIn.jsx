  ...
  useEffect(() => {
    if (userInfo) {
      if (userInfo.type === 'manager') {
        navigate('/manager');
      } else if (userInfo.type === 'pilot') {
        navigate('/pilot');
      }
      else if (userInfo.type === 'crew') {
        navigate('/crew');
      }
      else if (userInfo.type === 'engineer') {
        navigate('/engineer');
      }
    }
  }, [navigate, userInfo]);
      ...
      <div className="container mx-auto">
        <div className="flex flex-col lg:flex-row w-10/12 lg:w-8/12 bg-white rounded-xl mx-auto shadow-lg overflow-hidden">
          <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-12 bg-no-repeat bg-cover bg-center" style={{ backgroundImage: `url(${image})` }}>
            <h1 className="text-grey text-5xl font-extrabold font-mono mb-3">Sign In</h1>
          </div>
